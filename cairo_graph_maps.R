...
# Two separate maps:
# 1. Building area with bargraph as legend
# 2. Shape index with histogram legend using legendry
...
structured <- structured[!st_is_empty(structured), ]
unstructured <- unstructured[!st_is_empty(unstructured), ]
...
print(buildings$shape_index)

# calculate elongation ratio ( 1= aprroximately square, more values = more elongated)
buildings$elongation_ratio <- ifelse(
  is.finite(buildings$small_side) &
    is.finite(buildings$long_side) &
    buildings$small_side > 0,
  buildings$long_side /buildings$small_side,NA_real_)

# Add a temporary row ID
buildings$building_id <- seq_len(
  nrow(buildings)
)

# Split internally by urban type, calculate NND, and recombine
buildings <- buildings %<>%
  group_by(urban_type) %>%
  group_modify(
    ~ {
      ...
      # For each building:
      # neighbour 1 = itself
      # neighbour 2 = nearest other building
      neighbour_list <- nngeo::st_nn(x, x, k = 2, returnDist = FALSE, progress = FALSE)

      nearest_id <- vapply(
        neighbour_list,
        function(ids) {
          if (length(ids) >= 2) {ids[2]}
          else {NA_integer_}
         },
        integer(1)
      )

      x$nearest_distance_m <- NA_real_

      valid <- !is.na(nearest_id)

      x$nearest_distance_m[valid] <- as.numeric(
        st_distance(
          x[valid, ],
          x[nearest_id[valid], ],
          by_element = TRUE
...
# check the results
summary(buildings[c("area_m2", "shape_index", "elongation_ratio", "nearest_distance_m")])
...
# create a separate varibale to place the values above 1.8
...
ggsave("ShapeIndexMap1.png", map_shape_inset, width = 13, height = 10, dpi = 600)
...
# create a building area chloropleth map with barchart as a legend
...
# discret area histogram
hist_area <- ggplot(buildings, aes(x = area_class, fill = area_class)) +
  geom_bar() +
  scale_fill_manual(values = area_colours) +
  theme_minimal() +
  theme(
    ...
    panel.border = element_rect(color = "grey20", linewidth = 0.5, fill = NA)
  ) +
  labs(
...
# insert barchart into the building area map
layout_area <- map_area +
  inset_element(hist_area, left = 0.55, bottom = 0.75, right = 0.98, top = 0.98) +
  plot_annotation(
    caption = paste(
      "Basemap: © OpenStreetMap contributors, © CARTO",
      "CRS: WGS 84 / UTM Zone 36N (EPSG:32636)",
...
ggsave("Area_per_building1.png", layout_area, width = 13, height = 10, dpi = 600)
